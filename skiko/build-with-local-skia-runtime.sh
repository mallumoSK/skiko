...
SKIA_VERSION="$1" # Version of Skia m###-commit-sha-#. This commit sha will be cloned from repository https://github.com/JetBrains/skia
SKIA_DEBUG_MODE="$2" # in debug mode Skiko will be published with postix "+debug", for example "0.0.0-SNAPSHOT+debug"
SKIA_TARGET="$3" # possible values: "ios", "iosSim", "macos", "windows", "linux", "wasm", "android", "tvos", "tvosSim"
...
git clone "$SKIKO_PACK_URL" -b "$SKIKO_PACK_BRANCH" || echo "skia-pack exists. You can remove it or update by hands with git pull"
cd skia-pack
